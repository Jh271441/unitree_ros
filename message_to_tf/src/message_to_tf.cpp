...
void imuCallback(sensor_msgs::Imu const &imu) {
  ...
  if (g_pose_publisher) {
    ...
    tf::quaternionTFToMsg(rollpitch, pose_stamped.pose.pose.orientation);
    g_pose_publisher.publish(pose_stamped);
  }
}
...
int main(int argc, char** argv) {
  ...
  if (!g_odometry_topic.empty()) {
      sub1 = node.subscribe(g_odometry_topic, 10, &odomCallback, ros::TransportHints().tcpNoDelay());
      subscribers++;
  }
  ...
  if (publish_pose) {
    ...
    if (!publish_pose_topic.empty())
      g_pose_publisher = node.advertise<geometry_msgs::PoseWithCovarianceStamped>(publish_pose_topic, 10);
    else
      g_pose_publisher = priv_nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("pose", 10);
  }
